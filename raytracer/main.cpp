...
int main(int argc, char **argv)
{
	...
	std::thread t1([&num_threads, &main_disp, &image, &timer, &startTime] {
	...
	std::thread t2([&num_threads, &main_disp, &image, &timer, &startTime] {

		// Primary loop through all screen pixels.
		for (int y = 0; y < windowHeight /2; y++)
		{
			for (int x = 512; x <= windowWidth; x++)
			{
	...
	std::thread t3([&num_threads, &main_disp, &image, &timer, &startTime] {

		// Primary loop through all screen pixels.
		for (int y = 512; y < windowHeight; y++)
		{
			for (int x = 0; x < windowWidth / 2; x++)
			{
	...
	std::thread t4([&num_threads, &main_disp, &image, &timer, &startTime] {
